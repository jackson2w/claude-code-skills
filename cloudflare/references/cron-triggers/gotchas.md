...
### "Cron Fires On The Wrong Day"

**Problem:** A weekly cron intended for e.g. Monday actually fires on Sunday (or generally, one day earlier than expected)  
**Cause:** Cloudflare's day-of-week field is **1=Sunday..7=Saturday**, not the Unix convention (0 or 7=Sunday, 1=Monday) — writing `0 9 * * 1` expecting "Monday 9am" actually schedules "Sunday 9am", with no error anywhere to flag the mismatch  
**Solution:** Use the three-letter day names (`MON`, `TUE`, `SUN`, ...) instead of numeric day-of-week — this sidesteps the ambiguity entirely. Verify the actual registered schedule with `GET /accounts/{account_id}/workers/scripts/{script_name}/schedules` (or the dashboard's Triggers tab, which shows a human-readable "Next: ..." line) rather than assuming the numeric form means what it looks like it means.
