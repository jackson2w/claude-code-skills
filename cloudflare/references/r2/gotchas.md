...
**Cause:** R2 (via `wrangler r2 object`, the S3 API, or a Worker binding) has no move/rename call — only get/put/delete per key  
**Solution:** To rename or relocate a key: `get` it, `put` it at the new key, then `delete` the old one. There's no bulk version — scripting this per-object across many keys is expected for a multi-file rename, not a sign something's wrong.

### Delete/rename appears to not take effect

**Problem:** Right after a `delete` (or a get+put+delete rename) reports success, a direct `get`/`head` on that same key still returns 200 with the old content  
**Cause:** Read-after-delete consistency lag on direct object reads — observed lasting 15+ seconds in practice, even though the delete genuinely succeeded  
**Solution:** Don't verify a delete/rename with a single `get`/`head`. Instead list the bucket with a prefix filter (`objects?prefix=...` via the API, or `list()` in a Worker) — the list/index view reflects the true state immediately and consistently, even while direct reads are still lagging.
