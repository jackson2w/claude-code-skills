...
**Important nuance on "Functions override":** this isn't just "a Function can override a redirect if it handles the same path" — `_redirects` is skipped for a path even when the matching Function does nothing but call `next()` and pass through. Cloudflare's own docs: *"Redirects defined in the `_redirects` file are not applied to requests served by Pages Functions, even if the Function route matches the URL pattern."* A root `functions/_middleware.ts` (which matches every request) means `_redirects` never fires anywhere on that project, for any path — not just the ones the middleware actually acts on. If a project has a catch-all middleware and needs a redirect, implement it inside the middleware (`return Response.redirect(...)`) rather than `_redirects`.
