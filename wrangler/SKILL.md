...
- **Use environments for staging/prod**: Define `env.staging` and `env.production` in config. **Warning**: declaring a named env block — even one that only sets `vars` — stops Wrangler from inheriting top-level `kv_namespaces`/`r2_buckets`/other bindings into that environment. Every binding a named env needs must be re-declared inside its own block, or that environment silently deploys with those bindings missing (no error, just runtime failures on first binding access). See the note under "Full Config with Bindings" below.
...
```jsonc
{
  ...
  // Cron triggers. WARNING: day-of-week is 1=Sunday..7=Saturday here, NOT the
  // Unix convention (0/7=Sunday, 1=Monday) — "0 9 * * 1" is Sunday, not
  // Monday. Use three-letter names (MON, TUE, ...) to avoid the ambiguity
  // entirely. See the `cloudflare-workers-cron-email` skill for the full
  // diagnosis of a schedule that silently fired a day early because of this.
  "triggers": {
    "crons": ["0 9 * * MON"]
  },
  ...
  "env": {
    "staging": {
      ...
      // WARNING: this block only sets `vars`, so `kv_namespaces`, `r2_buckets`,
      // and every other top-level binding above are NOT inherited here —
      // deploying to staging ships with none of them bound. Re-declare every
      // binding this environment needs inside its own block:
      // "kv_namespaces": [{ "binding": "KV", "id": "<STAGING_KV_ID>" }],
      // "r2_buckets": [{ "binding": "BUCKET", "bucket_name": "my-bucket-staging" }]
    }
  }
}
```

**A named `env.*` block only gets what you put inside it — nothing is inherited from the top level once any named environment exists.** This is easy to miss because Wrangler deploys successfully and only warns in the log; the failure shows up later as every Function/Worker call that touches a binding throwing at runtime. If an environment's bindings are identical to the top level's, either duplicate them explicitly in that env block, or avoid named environments entirely (use separate `wrangler.toml` files per environment, or a single unnamed config, instead).
...
**No move/rename command exists.** To rename or relocate a key, `get` it to a temp file, `put` it at the new key, then `delete` the old one — there's no bulk operation, so scripting this per-object is normal for a multi-file rename.

**After a delete, a direct `get`/`head` on that key can still return the old content for 15+ seconds** — this looks like the delete failed but usually isn't. To verify a delete or bulk rename actually took effect, list the bucket with a prefix filter (`GET /accounts/{account_id}/r2/buckets/{bucket}/objects?prefix=...` via the REST API) instead of trusting a single `get`/`head` — the list endpoint reflects the true state immediately. See the `cloudflare-workers-cron-email` skill for the full story behind this.
