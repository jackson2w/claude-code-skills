...
### `crypto.subtle` PBKDF2 caps iterations at 100,000 on real Workers

`crypto.subtle.deriveBits`/`deriveKey` with PBKDF2 throws
`NotSupportedError: iteration counts above 100000 are not supported` when
run on an actual deployed Worker/Pages Function — but **not** in `wrangler
dev` or Miniflare, which use Node's crypto implementation and have no such
cap. Code hashing passwords at a commonly-cited higher iteration count (e.g.
210,000) will pass locally and 500 on every request once deployed.

**Check**: any `crypto.subtle.deriveBits`/`deriveKey` call with PBKDF2 — confirm
the iteration count is ≤100,000, and that this was verified against a real
deploy, not just local dev. If iteration count is stored alongside each
derived hash (recommended, so it can change later without invalidating
existing credentials), bumping it up to the cap is just a re-hash, not a
migration.

**Retrieve**: current limit at `/workers/platform/limits/` (subrequest/CPU
limits page also covers crypto constraints) — Cloudflare could raise this
cap over time.
