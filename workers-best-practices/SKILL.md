...
| PBKDF2 iteration cap | `crypto.subtle` PBKDF2 caps at 100,000 iterations on real Workers (no cap in local dev/Miniflare) — verify against a real deploy |
...
| PBKDF2 iterations > 100,000 | Passes in local dev (Node crypto, no cap), throws `NotSupportedError` on every request in production |
